...
function DrChatClientScr({delivery:dl,onBack}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",height:"100%"}}>
    <div className="chat-head"><button onClick={onBack} style={{width:36,height:36,borderRadius:10,border:"1px solid #E8E6E1",background:"#fff",cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>←</button><div style={{position:"relative"}}><div className="ch-av">👤</div><div style={{position:"absolute",bottom:-1,right:-1,width:10,height:10,borderRadius:"50%",background:"#10B981",border:"2px solid #fff"}}/></div><div className="ch-info"><h4>{dl.client.name}</h4><p style={{fontSize:11,color:"#908C82"}}>🟢 En ligne · 📍 {dl.client.addr.split(",")[0]} · {dl.ref}</p></div><button className="ch-call" onClick={()=>alert("📞 "+dl.client.phone)}>📞</button></div>
